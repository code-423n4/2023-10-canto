...
describe('Pool Conduit', () => {
    ...
    it("burn concentrated", async() => {
        await test.testMint(-2500, 8500, 5000)
        await test.testMint(-2500, 8500, 2000)
        expect(await conduit.hashMatches(baseToken.address, quoteToken.address, POOL_IDX)).to.be.true
        expect(await conduit.senderSnap_()).to.eq(await (await test.trader).getAddress())
        expect(await conduit.lowerSnap_()).to.eq(-2500)
        expect(await conduit.upperSnap_()).to.eq(8500)
    ...
    //     await test.testMint(-2500, 8500, 5000)
    //     await test.testSwap(true, true, 250000, MAX_PRICE)
    //     await test.testSwap(false, true, 250000, MIN_PRICE)
    //     await test.testMint(-2500, 8500, 5000)
