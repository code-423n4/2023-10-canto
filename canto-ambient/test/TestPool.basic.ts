...
describe('Pool', () => {
    ...
    //     await test.testMint(-1000, 2500, 100000);

    //     // Estabilish the pre-reward collateral commitment...
    //     let startQuote = await quoteToken.balanceOf((await test.dex).address)
    //     let startBase = await baseToken.balanceOf((await test.dex).address)
    //     await test.testBurn(-1000, 2500, 10000)
    ...
    //     await test.testSwap(true, true, 1000, toSqrtPrice(1.7))
    //     await test.testSwap(false, false, 10000, toSqrtPrice(1.5))
